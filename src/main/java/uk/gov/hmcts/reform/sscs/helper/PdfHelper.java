...
public class PdfHelper {
    ...
    public BigDecimal calculateScalingFactor(PDDocument document, PDRectangle size) {
        ...
        for (PDPage page: document.getPages()) {
            ...
            System.out.println("height limit = " + sizeHeight);
            System.out.println("width limit = " + sizeWidth);

            heightOverage = pageHeight - sizeHeight;
            System.out.println("height overage = " + heightOverage);
            widthOverage = pageWidth - sizeWidth;
            System.out.println("width overage = " + widthOverage);

            maxHeightScaling = maxScaleFactor(heightOverage, sizeHeight, maxHeightScaling);
            System.out.println("max height scaling = " + maxHeightScaling);
            maxWidthScaling = maxScaleFactor(widthOverage, sizeWidth, maxWidthScaling);
            System.out.println("max width scaling = " + maxWidthScaling);
        }

        BigDecimal scalingFactor = maxHeightScaling.compareTo(maxWidthScaling) > 0 ? maxHeightScaling : maxWidthScaling;

        return  scalingFactor.setScale(4, RoundingMode.HALF_EVEN);
    }

    public BigDecimal maxScaleFactor(float overage, float pageAxisDimension, BigDecimal maxScaleFactor) {
        if (overage > 0) {
